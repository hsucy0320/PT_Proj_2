...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
	...
	view.center = CGPointMake(view.bounds.size.width/2.0f, view.bounds.size.height/3.0f);
    view.superview.center = CGPointMake(self.bounds.size.width/2.0f + contentOffset.width,
										self.bounds.size.height/3.0f + contentOffset.height);
...
- (void)loadUnloadViews
{
    ...
    NSInteger max = numberOfItems - 1 + numberOfPlaceholdersToShow/2.0f;
    NSInteger count = MIN(numberOfVisibleItems, numberOfItems + numberOfPlaceholdersToShow);
    NSInteger offset = self.currentItemIndex - numberOfVisibleItems/2.0f;
    if (!shouldWrap)
    {
